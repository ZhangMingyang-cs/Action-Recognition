...
from Module import Regularization
from dataloader import NTUDataLoaders, AverageMeter, make_dir, get_cases, get_num_classes

args = argparse.ArgumentParser(description='ResNet')
args.add_argument('--model', type=str, default='resnet',
...
args.add_argument('--train', type=int, default=1,
                  help='train or test')
args.add_argument('--weight_decay', type=float, default=1e-2,
                  help='weight decay')
...
def main(results):
    ...
    if args.monitor == 'val_acc':
        ...
        best = np.Inf
        str_op = 'reduce'
    if args.dataset in ['NTU', 'NTU120', 'PKU']:
    ...
    best_epoch = 0
    output_dir = os.path.join('./results/CNN', args.dataset, args.model)
    ...
    if args.train == 1:
        for epoch in range(args.max_epoches):
            # train for one epoch
            t_start = time.time()
            train_loss, train_acc = train(train_loader, model, criterion, optimizer, epoch, reg_loss)
            # evaluate on validation set
            val_loss, val_acc = validate(val_loader, model, criterion, reg_loss)
...
def validate(val_loader, model, criterion, reg_loss=None):
    losses = AverageMeter()
    acces = AverageMeter()
...
if __name__ == '__main__':
    ...
    rootdir = os.path.join('./results/CNN', args.dataset, args.model)
